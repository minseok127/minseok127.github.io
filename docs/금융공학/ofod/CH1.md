...
중요한건 나는 100주를 이미 보유하고 있는 상황이니까 풋옵션을 매수하더라도 손익 그래프에서 기존 100주의 그래프에다가 옵션 매수로 인한 차익 그래프가 더해지는 식으로 생각해야함. 풋옵션의 그래프는 290달러 기준으로 왼쪽으로 갈수록 수익이 100x 기울기로 커지는데, 원래 현물의 손익 그래프는 100x 기울기로 작아지는거니까 상쇄됨. 근데 고정 비용은 2130달러임. 그러므로 만약 290달러보다 만기일 가격이 작다면 풋옵션 쓰는게 더 손해. 또한 290달러를 터치 안하고 만기일 가격이 더 상승했다면, 이 구간에서는 풋옵션의 손익 그래프는 상수값으로 마이너스라서 마찬가지 풋옵션 쓰는게 더 손해. 
