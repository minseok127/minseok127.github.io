...
먼저 풋옵션을 사용 안하고 손절하는 방식을 생각해보자. 290달러를 터치했다면, 만기일의 가격이 얼마든 상관없이 주당 26달러의 손해가 발생한다. 총합 2600달러의 손해. 만약에 터치하지 않았다면 만기일의 가격이 x라고 할 때 100 * (x - 316) 달러이다. 즉 $100x - 31600$

풋옵션을 사용하는 경우를 생각해보자. 여기서 살짝 헷갈렸던게 내가 이 문제 보기 전까지 옵션의 비용이 계약당 발생한다고 생각하고 있었다. 근데 알고 보니 주당 옵션 비용이었음. 즉 옵션 가격이 21.3달러인데, 이게 1계약 당 21.3달러가 아니라 1계약에 100주니까 2130달러가 옵션 매수 비용이었다. 그러면 고정 비용 2130달러이고, 만기일 가격 x가 290달러보다 크면 옵션 행사 안하니까 걍 2130달러 비용으로 나가는 것. 반대로 290달러보다 작으면 100 * (290 - x) 만큼의 수익이 나는 것. 즉 $29000 - 100x - 2130$.

중요한건 나는 100주를 이미 보유하고 있는 상황이니까 풋옵션을 매수하더라도 손익 그래프에서 기존 100주의 그래프에다가 옵션 매수로 인한 차익 그래프가 더해지는 식으로 생각해야함. 풋옵션의 그래프는 290달러 기준으로 왼쪽으로 갈수록 수익이 100x 기울기로 커지는데, 원래 현물의 손익 그래프는 100x 기울기로 작아지는거니까 상쇄됨. 근데 고정 비용은 2130달러가 추가된다. 그러므로 만약 290달러보다 만기일 가격이 작다면 풋옵션 쓰는게 더 손해. 또한 290달러를 터치 안하고 만기일 가격이 더 상승했다면, 이 구간에서는 풋옵션의 손익 그래프는 상수값으로 마이너스인 반면 원래 현물 손익은 100x 기울기 유지하면서 커지는 형태다. 즉 풋옵션 사면 그냥 현물 들고 있는 상황에서 비용만 추가로 나가는거니 마찬가지로 풋옵션 쓰는게 더 손해. 

근데 만약에 290달러를 터치했는데 만기일 가격은 이거보다 커지면? 이건 수식으로 봐야하는데, 손절한 경우는 항상 2600달러 손해인 상태. 풋옵션 매수의 경우는 원래 주식 보유의 경우 $100x - 31600$ 달러로 수익 그래프가 그려짐. 여기에 풋옵션은 행사 안하는 거니까 비용만 추가됨. 결국 $100x - 31600 - 2130$. 그러면 $100x - 31600 - 2130 = -2600$ 인 x를 찾으면 됨. 즉 311.3 달러를 기준으로 만기일 가격이 이거보다 작으면 손절의 케이스가 유리한거고, 이거보다 크면 풋옵션 매수가 유리한거.
