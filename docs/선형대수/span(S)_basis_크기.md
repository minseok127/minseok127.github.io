...
$span(S)$의 basis (기저)란 선형독립이면서 $spanning \ set$이 $span(S)$와 같은 벡터 집합이다. [선형종속과 선형독립]({% link docs/선형대수/선형종_선형독립.md %}) 페이지에 따르면 선형종속인 벡터 집합에 대해서 다른 벡터들의 선형결합으로 표현되는 벡터를 소거해도 $span$이 유지된다. 따라서 $S$가 선형종속이었다면 선형독립이 될 때까지 종속인 벡터들을 소거하며 basis를 찾을 수 있다. 
