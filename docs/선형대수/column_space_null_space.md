...
Column space부터 생각해보자. $C(A)$의 원소들은 $\mathbb{R}^m$에 있다. 이 집합에 속하는 원소들 $A\vec{x}, A\vec{y}$을 생각해보자. 벡터 $\vec{x}$의 $i$번째 성분을 $x_i$라고 하고 $A$의 $i$번째 열벡터를 $\vec{v_i}$라고 할 때, 두 원소는 다음과 같이 표현된다. 
