...
Column space부터 생각해보자. $C(A)$의 원소들은 $\mathbb{R}^m$에 있다. 이 집합에 속하는 원소들 $A\vec{x}, A\vec{y}$을 생각해보자. $A$의 $i$번째 열벡터를 $\vec{v_i}$라고 할 때, 두 원소는 다음과 같이 표현된다. 
...
위의 수식처럼 연산 결과인 $\alpha \vec{x} + \beta \vec{y}$ 또한 원래 $C(A)$ 에 속하는 원소들과 같은 방식으로 표현될 수 있기 때문에 $C(A)$는 덧셈과 스칼라곱에 닫혀있다.
