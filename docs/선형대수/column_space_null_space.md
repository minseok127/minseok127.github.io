...
위의 수식처럼 연산 결과인 $\alpha A\vec{x} + \beta A\vec{y}$ 또한 원래 $C(A)$ 에 속하는 원소들과 같은 방식으로 표현될 수 있기 때문에 $C(A)$는 덧셈과 스칼라곱에 닫혀있다. 연산 결과는 $C(A)$의 원소들로 계산한 것이기 때문에 $\mathbb{R}^m$에 속한다는 것에 주의하자.
...
연산 결과인 $\alpha \vec{x_{1}} + \beta \vec{x_{2}}$ 또한 $A$ 변환 후 영벡터가 되기 때문에 $N(A)$는 덧셈과 스칼라곱에 닫혀있다. 연산 결과는 $N(A)$의 원소들로 계산한 것이기 때문에 $\mathbb{R}^n$에 속한다는 것에 주의하자.
