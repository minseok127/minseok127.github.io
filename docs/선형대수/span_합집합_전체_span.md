...
정리하면 주어진 조건에서 $A \bigcup B = P$ 이면 $span(A) \bigcup span(B) = span(P)$ 라는 명제는 성립하지 않는다. 그리고 위의 과정에 나오는 것처럼 $span(P)$에 속하는 벡터 중에는 $span(A)$에도 속하지 않고 $span(B)$에도 속하지 않는 벡터가 있었다. 다르게 말하면 $A$와도 선형독립이고, $B$와도 선형독립인 벡터가 존재한다.
