...
$A \bigcup B = P$ 이기 때문에 $A$와 $B$는 $\vec{p_i}$ 들로 이뤄져있다. $A$에만 존재하고 $B$에는 존재하지 않는 벡터들을 $\vec{p_a}$라고 표기하고, $B$에만 존재하고 $A$에는 존재하지 않는 벡터들을 $\vec{p_b}$라고 표기하자. 그리고 $\vec{p_a}$의 계수를 $d_a$라고 하고, $\vec{p_b}$의 계수를 $d_b$ 라고 하자. $span(P)$ 에는 $d_a$ 중에 적어도 하나가 0이 아니면서 $d_b$ 중에서도 적어도 하나가 0이 아닌 벡터가 존재한다. 나머지 계수는 값이 상관없다. 이를 다음과 같이 표현해보자.
