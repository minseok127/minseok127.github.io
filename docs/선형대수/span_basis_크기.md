...
그런데 벡터들을 소거하는 순서는 정해져있지 않다. 예를 들어 $span(\lbrace (1,0), (1,1), (0,1) \rbrace)$를 생각해보면 아무 벡터나 먼저 소거해도 된다. 그러면 basis는 $\lbrace (1,1), (0,1) \rbrace$도 되고, $\lbrace (1,0), (0,1) \rbrace$도 되고, $\lbrace (1,0), (1,1) \rbrace$도 된다. 즉 소거하는 순서가 달라진다는 것은 basis가 되는 벡터 집합이 여러 개일 수 있다는 것이다. 이 basis들의 크기는 전부 동일할까?
