...
$span(B_P) \neq Q$ 이기 때문에 $Q$에 속하면서 $span(B_P)$에는 속하지 않는 벡터가 존재한다. 이 벡터를 $\vec{u_1}$라고 표현하자. 이 벡터와 $B_P$는 선형독립일까? 다음 수식을 생각해보자.
